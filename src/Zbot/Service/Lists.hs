...
listsCommand reply args = listsAction (T.words args)
    where
        ...
        showLists = do
            (MkLists listMap) <- get
            let speakKeys = lift . reply Direct <$> M.keys listMap
            case speakKeys of
                [] -> lift $ reply Direct "Such missing! Much not lists! Wow."
        ...
        showList l = do
            ...
            lift $ case result of
                Nothing -> reply Direct $ T.concat ["There is no list '", l, "'."]
                Just xs -> case xs of
                    []  ->  reply Direct "Much empty. Such void. Wow!"
                    _   ->  mapM_ (reply Direct . (\(i, (n, b)) -> T.concat [
                                    showText i, ") "
            ...
            case result of
                ...
                Just ls ->  do
                    ...
                    case result2 of
                        ...
                        Just _  -> lift $ reply Direct $ T.concat [
        ...
        addElemAt i l xs = do
            ...
            case result of
                ...
                Just ls -> do
                    ...
                    if b then lift $ reply Direct $ T.concat [
                                      "There is no index '", i
                                  ,   "' into list '", l, "'."
                                  ]
                         else do
            ...
            case result of
                ...
                Just ls -> do
                    ...
                    let result2 =   (,) result1
                                ...
                                <=  length ls
                    case result2 of
                        (Nothing,False) -> lift $ reply Direct
                                                $ T.concat [
                                                        "'", x
                                                    ,   "' does not exist "
                                                    ,   "within list '", l, "'."
        ...
        flushList l = do
            ...
            case result of
                ...
                Just _  -> do
                    ...
                    lift $ reply Direct $ T.concat ["Flushing '", l, "'."]
